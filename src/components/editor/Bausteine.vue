<template>
  ...
  -->
  <button id="info" :class="info" class="btn btn-outline-secondary" @click="info()"> i</button>
  <button id="weg" :class="weg" class="btn btn-outline-secondary" @click="waehle($event, weg)"> W</button>
  <button id="start" :class="start" class="btn btn-outline-secondary" @click="waehle($event, start)"> S</button>
  <button id="ziel" :class="ziel" class="btn btn-outline-secondary" @click="waehle($event,ziel)"> Z</button>

  <button id="schluessel" class="btn btn-outline-secondary" draggable="true"
          @click="setzeInfo(schluessel)" @dragstart="startDrag($event, schluessel)"><img src="/img/schluessel.png"
                                                                                         style="display: block; margin-left: auto; margin-right: auto;"/>
  </button>
  <button id="npc" class="btn btn-outline-secondary" draggable="true" @click="setzeInfo(npc)"
          @dragstart="startDrag($event, npc)"><img src="/img/npc.png"
                                                   style="display: block; margin-left: auto; margin-right: auto;"/>
  </button>
  <button id="tuer" class="btn btn-outline-secondary" draggable="true"
          @click="setzeInfo(tuer)" @dragstart="startDrag($event, tuer)"><img src="/img/tuer-h.png"
                                                                             style="display: block; margin-left: auto; margin-right: auto;"/>
  </button>
  <!--
  ...
  @dragstart="startDrag($event, raum)" @click="setzeInfo(raum)" id="räume"> R{{raum.title}} </button>
   -->
  <div id="raumausrichtung" class="btn-group btn-group-toggle" data-toggle="buttons">
    <p class="ueberschrift"> Tuerausrichtung </p>
    <label class="btn">
      <input autocomplete="off" checked name="options" type="radio" @click="ausrichten(0)">
      <p class="beschreibung">horizontal </p>
    </label>

    <label class="btn">
      <input autocomplete="off" name="options" type="radio" @click="ausrichten(1)">
